...
shinyServer(function(input, output, session) {
  ...
  observeEvent(vars1(), {
    ...
    updateSelectInput(inputId = "vis1_var", choices = vars1())
  })

  observeEvent(vars(), {
    freezeReactiveValue(input, "vis2_var1")
    freezeReactiveValue(input, "vis2_var2")
    updateSelectInput(inputId = "vis2_var1", choices = vars())
    updateSelectInput(inputId = "vis2_var2", choices = vars())
  })
  ...
  output$graphics_vis2 <- renderPlot({
    req(input$vis2_var1, input$vis2_var2)
    req(input$vis2_plot_type)
    ...
    switch(vis2_plot_flag(),
           TT = switch(input$vis2_plot_type,
             scatterPlot = plot(x, y, xlab=var1, ylab=var2),
             smoothScatterPlot = smoothScatter(x, y, xlab=var1, ylab=var2),
             sunflowerPlot = sunflowerplot(x, y, xlab=var1, ylab=var2),
           ),
           TF = switch(input$vis2_plot_type,
                       stripChart = plot(x, y, xlab=var1, ylab=var2),
                       spinePlot = spineplot(x, y, xlab=var1, ylab=var2),
                       cdPlot = cdplot(x, y, xlab=var1, ylab=var2),
           ),
           FT = switch(input$vis2_plot_type,
                       boxPlot = boxplot(x, y, xlab=var1, ylab=var2),
                       dotChart = dotchart(y, groups=x, xlab=var2, ylab=var1),
           ),
           FF = switch(input$vis2_plot_type,
                       spinePlot = spineplot(x, y, xlab=var1, ylab=var2),
                       stackedBarPlot = barplot(table(x, y), xlab=var1, ylab=var2, legend=rownames(table(x, y))),
                       dodgedBarPlot = barplot(table(x, y), beside = T, xlab=var1, ylab=var2, legend=rownames(table(x, y))),
                       assocPlot = assocplot(table(x, y), xlab=var1, ylab=var2),
                       mosaicPlot = mosaicplot(table(x, y), xlab=var1, ylab=var2, main=""),
           ),
    )
  })

  output$vis2_plot_type <- renderUI({

    plot_types <- switch(vis2_plot_flag(),
           TT = c("散点图"="scatterPlot", "光滑散点图"="smoothScatterPlot", "向日葵散点图"="sunflowerPlot"),
           TF = c("散列图"="stripChart", "脊柱图"="spinePlot", "条件密度图"="cdPlot"),
           FT = c("箱线图"="boxPlot", "点图"="dotChart"),
           FF = c("脊柱图"="spinePlot", "堆砌条形图"="stackedBarPlot", "分组条形图"="dodgedBarPlot", "关联图"="assocPlot", "马赛克图"="mosaicPlot")
           )
    selectInput("vis2_plot_type", "选择绘图类型：", multiple=FALSE, choices=plot_types)
  })
})
