              ...
              conditionalPanel("input.options_vis1==true",
                               ...
                               column(width=4,
                                      conditionalPanel("input.vis1_var_type == 'numeric' & input.vis1_num_plot_type == 'scatter_t'",
                                                       selectInput("vis1_col", label="颜色", choices=colors(), selected="black")
                                      ),

                                      conditionalPanel("input.vis1_var_type != 'factor' | input.vis1_factor_plot_type != 'pie_t'",
                                                       selectInput("vis1_bg", label="填充颜色", choices=colors(), selected="lightgray")
                                      ),

                                      conditionalPanel("input.vis1_var_type == 'numeric' & input.vis1_num_plot_type == 'scatter_t'",
                                                       numericInput("vis1_pch", label="形状", min=0, max=25, value=1, step=1),
                                                       numericInput("vis1_cex", label="大小", min=0.1, max=10, value=1)
                                      ),

                                      conditionalPanel("input.vis1_var_type == 'numeric' & input.vis1_num_plot_type == 'histogram_t'",
                                                       numericInput("vis1_breaks", label="分箱数", min=1, max=30, value=5, step=1),
                                      ...
                                      conditionalPanel("input.vis1_var_type == 'factor' & input.vis1_factor_plot_type == 'pie_t'",
                                                       numericInput("vis1_radius", label="半径", min=.1, max=1, value=.8, step=.1),
                                                       numericInput("vis1_angle", label="初始角度", min=0, max=360, value=0, step=10),
