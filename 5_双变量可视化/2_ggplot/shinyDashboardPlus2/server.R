...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$vis_params_ok, {
    ...
    vis_params$opts$show_legend <- input$vis_show_legend
    vis_params$opts$color_title <- input$vis_color_title
    vis_params$opts$fill_title <- input$vis_fill_title
    vis_params$opts$char_fill_title <- input$vis_char_fill_title
    ...
    } else if (input$vis_tab == 'Vis2') {
      if (input$vis2_plot_type == "scatter_t") {
        vis_params$opts2$scatter_margin_type <- input$vis2_scatter_margin_type
        vis_params$opts2$scatter_margins     <- input$vis2_scatter_margins
        vis_params$opts2$add_rug             <- input$vis2_add_rug
      } else if (input$vis2_plot_type == "smooth_scatter_t") {
        vis_params$opts2$add_scatter       <- input$vis2_add_scatter
        vis_params$opts2$user_palette      <- input$vis2_user_palette
        vis_params$opts2$user_palette_low  <- input$vis2_user_palette_low
        vis_params$opts2$user_palette_high <- input$vis2_user_palette_high
      } else if (input$vis2_plot_type == "density2d_t") {
        vis_params$opts2$add_scatter       <- input$vis2_add_scatter
      } else if (input$vis2_plot_type == "density_t") {
        vis_params$opts2$position          <- input$vis2_position
      } else if (input$vis2_plot_type == 'boxplot_t') {
        vis_params$opts2$width            <- input$vis2_width
        vis_params$opts2$notch            <- input$vis2_notch
        vis_params$opts2$horizontal       <- input$vis2_horizontal
        vis_params$opts2$box_fill         <- input$vis2_box_fill
        vis_params$opts2$add_jitter       <- input$vis2_add_jitter
        vis_params$opts2$jitter_width     <- input$vis2_jitter_width
      } else if (input$vis2_plot_type == 'violin_t') {
        vis_params$opts2$width            <- input$vis2_width
        vis_params$opts2$horizontal       <- input$vis2_horizontal
        vis_params$opts2$box_fill         <- input$vis2_box_fill
        vis_params$opts2$add_jitter       <- input$vis2_add_jitter
        vis_params$opts2$jitter_width     <- input$vis2_jitter_width
        vis_params$opts2$add_boxplot      <- input$vis2_add_boxplot
      } else if (input$vis2_plot_type == 'barplot_t') {
        vis_params$opts2$position          <- input$vis2_position
        vis_params$opts2$width             <- input$vis2_width
        vis_params$opts2$horizontal        <- input$vis2_horizontal
      }
  ...
  observeEvent(input$vis_params_clear, {
    ...
    vis_params$opts2 <- init_vis2_options()
  })

  # 单变量可视化图像保存为反应表达式
  vis1 <- reactive({
    req(input$vis1_var, input$vis1_var_type)
    req(check_palatte_discrete(vis_params$global$palette_family_d,
                               vis_params$global$palette_d))
...
#  output$message <- renderPrint({})
  ...
  vis2 <- reactive({
    ...
    req(input$vis2_var1, input$vis2_var2, input$vis2_plot_type)

    req(check_palatte_discrete(vis_params$global$palette_family_d,
                               vis_params$global$palette_d))
    req(check_palatte_continuous(vis_params$global$palette_family_c,
                                 vis_params$global$palette_c))

    req(input$vis2_var2 %in% vis2_vars())

    vis2_plot(dataset(), input$vis2_var1, input$vis2_var2, input$vis2_plot_type, vis_params)
  })
  ...
  vis2_state <- reactiveValues(
    dataset=NULL,
    type=NULL
  )

  observeEvent(input$vis2_var1, {

    var1 <- input$vis2_var1
    var2 <- input$vis2_var2

    freezeReactiveValue(input, "vis2_var2")

    var2_choices <- setdiff(vis2_vars(), var1)

    if (is.null(vis2_state$dataset)) {
      vis2_state$dataset <- input$dataset
      updateSelectInput(inputId="vis2_var2", choices=var2_choices)
    } else {
      if (vis2_state$dataset!=input$dataset) {
        vis2_state$dataset <- input$dataset
        updateSelectInput(inputId="vis2_var2", choices=var2_choices)
      } else {
        if (!is.null(var2) & var2!=input$vis2_var1) {
          updateSelectInput(inputId="vis2_var2", choices=var2_choices, selected=var2)
        } else {
          updateSelectInput(inputId="vis2_var2", choices=var2_choices)
        }
      }
  ...
  observeEvent(input$vis2_var2, {
    req(input$vis2_var1, input$vis2_var2)

    vis2_type <- get_vis2_type(dataset()[[input$vis2_var1]],
                               dataset()[[input$vis2_var2]])

    if (is.null(vis2_state$type)) {
      vis2_state$type <- vis2_type
      freezeReactiveValue(input, "vis2_plot_type")
      updateSelectInput(inputId="vis2_plot_type", choices=vis2_types[[vis2_type]])
    } else {
      if (vis2_state$type!=vis2_type) {
        vis2_state$type <- vis2_type
        freezeReactiveValue(input, "vis2_plot_type")
        updateSelectInput(inputId="vis2_plot_type", choices=vis2_types[[vis2_type]])
      }
