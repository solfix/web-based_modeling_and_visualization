  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      wellPanel(
        ...
        #  (2-1a-3) conditionalPanel()动态显示UI：如果应用数据筛选，显示
        #           确定筛选条件的构件，否则隐藏构件。
        #    (2-1a-3a) 单选selectInput()创建筛选变量；
        #    (2-1a-3b) uiOuput()结合make_filter_ui()，动态创建筛选构件：
        #              对于数值型筛选变量，sliderInput()创建滑块构件；
        #              对于因子型筛选变量，selectInput()创建多选构件。
        conditionalPanel("input.show_filter == true",
                         selectInput("filter_var", "选择筛选变量：",
                                     choices = names(mtcars)[purrr::map_lgl(mtcars, ~is.numeric(.) | is.factor(.))]),
                         uiOutput("filter")),
      ),
...
server <- function(input, output) {
  ...
  observeEvent(dataset(), {
    ...
    freezeReactiveValue(input, "filter_var")

    # 以新数据集的变量名更新变量（列）的选择项
    updateSelectInput(inputId = "columns", choices = names(dataset()), selected = names(dataset()))

    # 以新数据集的变量名更新筛选变量选择项
    updateSelectInput(inputId = "filter_var", choices = names(dataset())[purrr::map_lgl(dataset(), ~is.numeric(.) | is.factor(.))])

  })

  # 根据不同变量的类型及其取值范围动态构建筛选UI控件
  output$filter <- renderUI({
    x <- dataset()[[input$filter_var]]
    make_filter_ui(x, input$filter_var)
  })

  # 用函数filter_var()设置筛选条件
  filter_selected <- reactive({
    x <- dataset()[[input$filter_var]]
    val <- input[[input$filter_var]]
    filter_var(x, val)
  })
