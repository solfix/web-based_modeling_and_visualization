...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        sidebarPanel(
            selectInput("dataset", "选择数据集：", choices=df_names, selected="mtcars"),
            selectInput("columns", "选择变量：", multiple=TRUE, choices=names(mtcars), selected=names(mtcars))
        ),

        # (2-2) 主面版---输出控件reactableOutput以表格显示数据
        mainPanel(
...
server <- function(input, output) {
    ...
    # renderReactable显示数据集中已选取的变量，缺省一页显示5行
    output$table <- renderReactable({
        dataset()[, input$columns] %>% reactable()
    })
}
