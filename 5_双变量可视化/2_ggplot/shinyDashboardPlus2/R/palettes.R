...
ggsci_palettes_continuous <- c(
  "gsea" = "gsea", # The GSEA GenePattern Color Scales
...
# 检查调色板族和调色板是否一致
check_palatte_discrete <- function(palatte_class, palatte_name) {
  switch (palatte_class,
          base = (palatte_name %in% c("none", base_palettes)),
          RColorBrewer = (palatte_name %in% brewer_palettes),
          viridis = (palatte_name %in% viridis_palettes),
          wesanderson = (palatte_name %in% wesanderson_palettes),
          ggsci = (palatte_name %in% ggsci_palettes_discrete),
          FALSE
  )
}

check_palatte_continuous <- function(palatte_class, palatte_name) {
  switch (palatte_class,
          base = (palatte_name %in% c("none", base_palettes)),
          viridis = (palatte_name %in% viridis_palettes),
          wesanderson = (palatte_name %in% wesanderson_palettes),
          ggsci = (palatte_name %in% ggsci_palettes_continuous),
          FALSE
  )
}
