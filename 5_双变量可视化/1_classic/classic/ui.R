...
# (1) 标题
header <- dashboardHeader(title = "双变量可视化")
...
sidebar <- dashboardSidebar(
  ...
  div(
    sidebarMenu(id = "sidebar",
                ...
                menuItem("双变量可视化", tabName = "Vis2", selected = TRUE, icon = icon("chart-line"))
    )
  ),
  ...
  conditionalPanel("input.sidebar == 'Vis2'",
                   selectInput("vis2_var1", "选择 X 变量：", multiple=FALSE, choices=NULL),
                   selectInput("vis2_var2", "选择 Y 变量：", multiple=FALSE, choices=NULL),
  )
    ...
    tabItem(tabName = "Vis2",
            fluidPage(
              tabBox(width = 12,
                     tabPanel(title = "graphics",
                              fluidRow(
                                box(width = 12, plotOutput("graphics_vis2"), solidHeader = TRUE)
                                ),
                              fluidRow(
                                box(width = 4, uiOutput("vis2_plot_type"), solidHeader = TRUE)
                                )
                              ),
              )
            )
